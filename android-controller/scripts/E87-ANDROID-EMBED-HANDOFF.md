...
The APK source path is read once into an immutable byte snapshot before authorization. ZIP, DEX-build, and receipt hashes consume those bytes. On the required Linux verification host, a kernel-sealed `memfd` holds that same snapshot; the seal forbids write, truncate, grow, and shrink operations even by the owning UID. A short reviewed helper inherits that descriptor inside a fresh unprivileged user/mount namespace, copies and re-hashes it at `/mnt/e87-controller-snapshot.apk` on a new `tmpfs`, remounts the entire `tmpfs` read-only, and only then executes the exact `aapt`, `dexdump`, or `adb install` command. Both `/` and `/mnt` are revalidated as root-owned, non-symlink directories with no group/other write bits, and the invoking host UID must be non-root; unlike a caller-owned temporary directory, that fixed anchor cannot be renamed and replaced by the caller UID after the mount. This supports tools that reject `/proc` descriptor symlinks without reopening a mutable host path. The commands fail closed when the protected anchor, sealed-memory, or unprivileged namespace support is unavailable. Replacing or restoring either the caller's APK path or the subprocess projection cannot change the bytes installed or recorded. Installed verification likewise retains the expected snapshot while pulling, snapshots the pulled base APK once, then compares and re-audits those exact bytes.
